...
<title>News Checker By 대한민국 해시(#)포럼</title>
<style>
  :root {{
...
<body>
<header>
  <h1>News Checker By 대한민국 해시(#)포럼</h1>
...
function renderItemMatrix(sortKey) {{
  ...
  const bodyRows = entries.map(([code, d], i) => {{
    const cells = ITEM_KEYS.map(k => {{
      if (ITEM_UNIMPL.has(k))
        return `<td onclick="event.stopPropagation();toggleItemDetail('${{code}}','${{k}}')" style="cursor:pointer">`
             + `<span class="m-cell cnt0" style="color:#e0e0e0">·</span></td>`;
      const n = d.items[k].length;
      return `<td onclick="event.stopPropagation();toggleItemDetail('${{code}}','${{k}}')" style="cursor:pointer">`
           + `<span class="m-cell ${{cntClass(n)}}">${{n > 0 ? n : '·'}}</span></td>`;
    }}).join('');
    return `
      <tr class="matrix-row" onclick="toggleItemDetail('${{code}}')">
        ...
        <td>${{esc(d.name)}}</td>
        ${{cells}}
        <td onclick="event.stopPropagation();toggleItemDetail('${{code}}')" style="cursor:pointer"><strong>${{d.total}}</strong></td>
      </tr>
...
function toggleItemDetail(code, filterKey) {{
  const row    = document.getElementById('idetail-' + code);
  const isOpen = row.style.display !== 'none';
  const prev   = row.dataset.filter || '';
  const next   = filterKey || '';
  if (isOpen && prev === next) {{
    row.style.display = 'none';
    row.dataset.filter = '';
  }} else {{
    row.style.display = 'table-row';
    row.dataset.filter = next;
    renderItemDetail(code, next || null);
  }}
}}
...
function renderItemDetail(code, filterKey) {{
  const d = _matrix[code];
  if (!d) return;

  const keysToShow = filterKey ? [filterKey] : ITEM_KEYS;
  const sections = keysToShow
